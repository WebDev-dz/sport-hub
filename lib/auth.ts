...
import { stripe, StripePlan } from "@better-auth/stripe";
  ...
  plugins: [
    ...
    stripe({
      ...
      subscription: {
        ...
        plans: async () => {
          const client = new Stripe(process.env.STRIPE_KEY || "sk_test_");
          const prices = await client.plans.list();
        ...
        //   };
		  ...
		  const data = prices.data.map((pr) => ({
			...pr,
			name: pr.product?.toString()!
		  }))
